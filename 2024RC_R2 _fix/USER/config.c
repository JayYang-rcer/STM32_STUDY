...
{
    PID_Parameter_Init(&MOTOR_PID_RPM[0], 12.0, 2.0, 0.2 , 10000 ,10 , 0 , 500);	//0x201号电机 M2006,C620电调驱动下，14A是电机最高的工作电流，额定电流是10A
	PID_Parameter_Init(&MOTOR_PID_RPM[1], 9.0, 1.0, 2.1 , 10000 ,10 , 0 , 500);	//0x202号电机 M3508
	PID_Parameter_Init(&MOTOR_PID_RPM[2], 12.0, 4.0, 0.2 , 10000 ,10 , 0 , 500);	//0x203号电机 M3508
	PID_Parameter_Init(&MOTOR_PID_RPM[3], 9.0, 2.0, 0.3 , 10000 ,10 , 0 , 500);	//0x204号电机 M3508
...
void VESC_Init(void)
{
    ...
    VESC_MOTO_INFO[0].VESC_CAN_ID = 101;            //电调ID采用 101，102，103.....

    VESC_MOTO_INFO[1].MotorType = VESC_5065;
    VESC_MOTO_INFO[1].VESC_CAN_ID = 102;

    VESC_MOTO_INFO[2].MotorType = VESC_5065;
    VESC_MOTO_INFO[2].VESC_CAN_ID = 103;

    VESC_MOTO_INFO[3].MotorType = VESC_5065;
    VESC_MOTO_INFO[3].VESC_CAN_ID = 104;
}
